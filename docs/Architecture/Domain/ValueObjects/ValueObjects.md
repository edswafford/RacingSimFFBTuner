...
ShockVelocity ✅ COMPLETED

TireLoad ✅ COMPLETED

WheelPosition ✅ COMPLETED

WheelVelocity ✅ COMPLETED

ForceFeedbackVector ✅ COMPLETED

Scale ✅ COMPLETED

PidConfig ✅ COMPLETED

YawRateFactor ✅ COMPLETED

TelemetryDataPoint ✅ COMPLETED
...
✅ YawRate - COMPLETED - Angular velocity (radians/second)

✅ Speed - COMPLETED - Velocity magnitude

✅ Torque - COMPLETED - Force feedback torque in Newton-meters

✅ Velocity - COMPLETED - 3D velocity vector (X, Y, Z components)

✅ GForce - COMPLETED - Calculated from velocity change

✅ EngineRPM - COMPLETED - Engine rotational speed

Phase 2: Telemetry Components (Depend on Phase 1)

✅ ShockVelocity - COMPLETED - Suspension shock velocity per corner

✅ TireLoad - COMPLETED - Tire force and slip ratio

Phase 3: Hardware Input (Independent)

✅ WheelPosition - COMPLETED - Raw hardware wheel position

✅ WheelVelocity - COMPLETED - Calculated from wheel position

Phase 4: FFB Output (Depend on Torque)

✅ ForceFeedbackVector - COMPLETED - 3D force vector (X, Y, Z, Damper, Inertia)

Phase 5: Configuration (Independent)

✅ Scale - COMPLETED - FFB scale values (overall, detail, LFE)

✅ PidConfig - COMPLETED - PID controller parameters

Phase 6: Calculated Metrics (Depend on Generic)

✅ YawRateFactor - COMPLETED - Calculated from steering angle, speed, yaw rate (for understeer detection)

✅ TelemetryDataPoint - COMPLETED - Composite value object containing all telemetry

Pattern to Follow

All value objects should follow the pattern established in SteeringWheelAngle.cs:

Properties first (IDesign ordering)

Constructor after properties with validation

Public methods before private validation methods

XML documentation comments (even though SA0001 is disabled, existing code has them)

File ends with single newline (SA1518)

Immutable - Use readonly record struct

Validation - Reject NaN, Infinity, and out-of-range values

Value equality - Two instances with same values are equal
...
Valid construction - Normal cases with valid inputs

Boundary conditions - Minimum, maximum, zero values

Invalid inputs - NaN, Infinity, out-of-range values

Equality - Value equality, not reference equality

Conversion methods - If applicable (e.g., radians to degrees)

Factory methods - If applicable (e.g., FromDegrees)
...
Reference: LegacySimRacingFFB.md (line 260-262)

Validation Rules

Range: No specific physical limits (can be any real number)

Invalid values: Reject NaN and Infinity
...
Reference: LegacySimRacingFFB.md (line 264-266)

Validation Rules

Range: No specific physical limits (can be any real number for each component)

Invalid values: Reject NaN and Infinity for any component
...
Range: Must be >= 0 (speed cannot be negative)
...
Reference: LegacySimRacingFFB.md (line 268-270, 104-106)

Validation Rules

Range: Must be between -32 and +32 G-forces (safety limit)
...
Reference: LegacySimRacingFFB.md (line 248-250)
...
Range: No specific physical limits (can be positive or negative)
...
Range: Must be >= 0 (RPM cannot be negative)

Invalid values: Reject NaN and Infinity

Units: Revolutions per minute (RPM)

Optional: MaxRPM limit for validation (car-specific)
...
Reference: LegacySimRacingFFB.md (line 252-254, 108-110)
...
Load Range: Must be >= 0 (force cannot be negative)

Slip Ratio Range: Typically -1 to 1, but can exceed in extreme cases

Invalid values: Reject NaN and Infinity for both properties
...
Reference: LegacySimRacingFFB.md (line 303-305)

Validation Rules

Range: Typically -32768 to 32767 for DirectInput, but should be flexible

Invalid values: No NaN/Infinity (int type)
...
Reference: LegacySimRacingFFB.md (line 307-309)

Validation Rules

Range: No specific limits (can be positive or negative)
...
Range: No specific limits for force components (can be positive or negative)

Damper Range: Must be >= 0 (damping cannot be negative)

Inertia Range: Must be >= 0 (inertia cannot be negative)

Invalid values: Reject NaN and Infinity for all components
...
Range: Typically 0-200% (can exceed 100% for over-scaling)
...
Range: No specific limits (gains can be positive or negative depending on control strategy)

Invalid values: Reject NaN and Infinity for all gains

Units: Unitless gains (proportional, integral, derivative)
...
Reference: LegacySimRacingFFB.md (line 100-102)
...
Calculation constraints: Only valid when yaw rate >= 5 degrees/second and speed > 0
...
Reference: LegacySimRacingFFB.md (line 226-228)
...
Required components: All core telemetry value objects must be present

Optional components: Some values may be optional (e.g., when not on track)

Timestamp: Should include a timestamp for ordering/sequencing
...
iRacing-Specific Considerations: iRacing-Specific Value Objects.md
...
Begin implementing Phase 1 value objects using TDD

Reference iRacing-Specific Value Objects.md for simulator-specific implementation details that cannot be generalized